      ...
      <div className="pt-32 pb-24 px-6 max-w-6xl mx-auto relative z-20">
        ...
        <div className="text-center mb-16">
          <h1 className="text-4xl md:text-6xl font-bold mb-6 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">
            {settings?.contact_heading || (i18n.language === 'ar' ? 'اتصل بنا' : 'Contact Us')}
          </h1>
          <p className="muted-text text-xl max-w-2xl mx-auto">
            {settings?.contact_description || (i18n.language === 'ar'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          <div className="space-y-12">
            <div>
              <h3 className="text-2xl font-semibold mb-8 border-b border-[var(--border-subtle)] pb-4">
                {settings?.contact_information_heading || (i18n.language === 'ar' ? 'معلومات التواصل' : 'Contact Information')}
              </h3>
              ...
              <div className="space-y-8">
                <div className="flex items-start gap-6">
                  ...
                  <div>
                    <h4 className="soft-text text-lg font-medium mb-1">
                      {settings?.contact_phone_label || (i18n.language === 'ar' ? 'اتصل بنا اليوم' : 'Call us today')}
                    </h4>
                    <a href={`tel:${settings?.contact_phone || '+1(202)9984099'}`} className="text-xl font-semibold hover:text-cyan-400 transition-colors" dir="ltr">
                      {settings?.contact_phone || '+1(202)9984099'}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-6">
                  ...
                  <div>
                    <h4 className="soft-text text-lg font-medium mb-1">
                      {settings?.contact_email_label || (i18n.language === 'ar' ? 'البريد الإلكتروني' : 'Our emails')}
                    </h4>
                    <a href={`mailto:${settings?.contact_email || 'info@sarab.tech'}`} className="text-xl font-semibold hover:text-cyan-400 transition-colors">
                      {settings?.contact_email || 'info@sarab.tech'}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-6">
                  ...
                  <div>
                    <h4 className="soft-text text-lg font-medium mb-1">
                      {settings?.contact_address_label || (i18n.language === 'ar' ? 'عنواننا' : 'Our address')}
                    </h4>
                    <p className="muted-text text-lg leading-relaxed" dir="ltr">
                      {settings?.address || '175 SW 7th Street, Suite 1517 - 1197, Miami, Florida 33130'}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="form-surface rounded-3xl p-8 md:p-12 backdrop-blur-sm border">
            <h3 className="text-2xl font-semibold mb-8">
              {settings?.contact_form_heading || (i18n.language === 'ar' ? 'أرسل لنا رسالة' : 'Send us a message')}
            </h3>
            ...
            <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label htmlFor="firstName" className="muted-text text-sm font-medium">
                    {settings?.contact_first_name_label || (i18n.language === 'ar' ? 'الاسم الأول' : 'First Name')}
                  </label>
                  <input 
                    type="text" 
                    id="firstName" 
                    ...
                    placeholder={settings?.contact_first_name_placeholder || (i18n.language === 'ar' ? 'أدخل اسمك الأول' : 'John')}
                  />
                </div>
                <div className="space-y-2">
                  <label htmlFor="lastName" className="muted-text text-sm font-medium">
                    {settings?.contact_last_name_label || (i18n.language === 'ar' ? 'اسم العائلة' : 'Last Name')}
                  </label>
                  <input 
                    type="text" 
                    id="lastName" 
                    className="input-surface w-full border rounded-xl px-4 py-3 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all"
                    placeholder={settings?.contact_last_name_placeholder || (i18n.language === 'ar' ? 'أدخل اسم العائلة' : 'Doe')}
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                <label htmlFor="email" className="muted-text text-sm font-medium">
                  {settings?.contact_email_input_label || (i18n.language === 'ar' ? 'البريد الإلكتروني' : 'Email Address')}
                </label>
                <input 
                  type="email" 
                  id="email" 
                  className="input-surface w-full border rounded-xl px-4 py-3 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all"
                  placeholder={settings?.contact_email_placeholder || 'john@example.com'}
                  dir="ltr"
                />
              </div>
              ...
              <div className="space-y-2">
                <label htmlFor="subject" className="muted-text text-sm font-medium">
                  {settings?.contact_subject_label || (i18n.language === 'ar' ? 'الموضوع' : 'Subject')}
                </label>
                <input 
                  type="text" 
                  id="subject" 
                  className="input-surface w-full border rounded-xl px-4 py-3 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all"
                  placeholder={settings?.contact_subject_placeholder || (i18n.language === 'ar' ? 'كيف يمكننا مساعدتك؟' : 'How can we help you?')}
                />
              </div>

              <div className="space-y-2">
                <label htmlFor="message" className="muted-text text-sm font-medium">
                  {settings?.contact_message_label || (i18n.language === 'ar' ? 'الرسالة' : 'Message')}
                </label>
                <textarea 
                  id="message" 
                  rows={5}
                  ...
                  placeholder={settings?.contact_message_placeholder || (i18n.language === 'ar' ? 'اكتب رسالتك هنا...' : 'Write your message here...')}
                ></textarea>
              </div>
              ...
              >
                ...
                {settings?.contact_form_submit_label || (i18n.language === 'ar' ? 'إرسال الرسالة' : 'Send Message')}
              </button>
            </form>
          </div>
        </div>
